...
class TestLambdaHandler(unittest.TestCase):
    ...
    def invoke_lambda_handler(event_args: Union[dict, None]) -> dict:
        event = TestLambdaHandler.api_gateway_event_stub(
            event_args) if event_args else dict()
    ...
    def test_uncaught_victoria_exceptions(self):
        ...
        response = self.invoke_lambda_handler(
            {'args': ['store', 'unknownstore', 'ls', 'ls']})
        self.assertEqual(response['statusCode'],
                         HTTPStatus.INTERNAL_SERVER_ERROR)
        self.assertEqual(
            response['error'],
            "Command failed: uncaught Victoria exception detected")
    ...
    def test_handled_victoria_errors(self):
        ...
        response = self.invoke_lambda_handler(
            {'args': ['store', 'wrong', 'command']})
        self.assertEqual(response['statusCode'],
                         HTTPStatus.INTERNAL_SERVER_ERROR)
        self.assertEqual(response['error'],
                         'Command failed: non-zero status code')
